...
unwrap :: Concurrent a -> ((a -> Action) -> Action)
unwrap (Concurrent x) = x

-- action :: Concurrent a -> Action
-- action = \x -> unwrap x (const Stop) 
...
--         return (c a))) 
atom :: IO a -> Concurrent a
atom f = Concurrent (\cont -> Atom (f >>= (\a -> return (cont a)))) 
...
fork cont = Concurrent(\x -> Fork (action cont) (x ()))

par :: Concurrent a -> Concurrent a -> Concurrent a
par (Concurrent a1) (Concurrent a2) = Concurrent(
   \cont -> Fork (a1 cont) (a2 cont)) 
...
instance Monad Concurrent where
    (Concurrent f) >>= g = Concurrent (\x -> f (\y -> (unwrap (g y)) x))
